...
public class ReservaDeEntradas extends javax.swing.JFrame {
    ...
    String sql = "SELECT f.ID_HORARIO, " +
                ...
                "WHERE f.ID_PELICULA = ? AND " +
                "CONCAT(f.FECHA_FUNCION, ' ', f.FUNCION_INICIO) >= NOW() " +
    ...
    try (Connection conn = ConexionBD.getConnection();
        ...
        if (!hayFunciones) {
            JOptionPane.showMessageDialog(this, 
                "No hay funciones disponibles para esta película. Todas las funciones están llenas.",
    ...
    String sql = "SELECT s.ID_SALA, s.NUMERO_SALA FROM sala s " +
    ...
    String sql = "SELECT (f.ASIENTOS_TOTALES_FUNCION - COUNT(b.ID_BUTACAS)) as DISPONIBLES " +
                "FROM funcion f LEFT JOIN butaca b ON f.ID_HORARIO = b.ID_HORARIO " +
                "WHERE f.ID_HORARIO = ? GROUP BY f.ID_HORARIO";
