...
public class AsignarHorarios extends javax.swing.JFrame {
    ...
    configurarTabla();
    cargarSucursales(); // Esto iniciará la carga en cascada cuando se seleccione una sucursal
}
    ...
    cargarFuncionesPorSucursal();
}


    private void cargarCombos() {
        cargarPeliculasPorSucursal();
        cargarSalasPorSucursal();
        cargarSucursales();
    }

    private void cargarPeliculasPorSucursal() {
    if (combo_sucursal.getSelectedItem() == null) return;
    
    int idSucursal = ((Item)combo_sucursal.getSelectedItem()).getId();
    String sql = "SELECT DISTINCT p.ID_PELICULA, p.NOMBRE_PELICULA " +
                 "FROM pelicula p " +
   ...
   private void cargarSalasPorSucursal() {
    if (combo_sucursal.getSelectedItem() == null) return;
    
    int idSucursal = ((Item)combo_sucursal.getSelectedItem()).getId();
    String sql = "SELECT ID_SALA, NUMERO_SALA FROM sala WHERE ID_SUCURSAL = ? AND ESTADO_SALA = 1";
    ...
    try (Connection conn = ConexionBD.getConnection();
        ...
        combo_sucursal.addItemListener(e -> {
    ...
    private void cargarFuncionesPorSucursal() {
    if (combo_sucursal.getSelectedItem() == null) return;
    ...
    String sql = "SELECT f.ID_HORARIO, p.NOMBRE_PELICULA, f.FECHA_FUNCION, " +
                 ...
                 "WHERE s.ID_SUCURSAL = ? " +
                 "ORDER BY f.FECHA_FUNCION, f.FUNCION_INICIO";
 ...
 private boolean existeFuncionDuplicada(int idSala, String fecha, String horaInicio, String horaFin) {
    // Primera validación: función idéntica
    String sql1 = "SELECT COUNT(*) FROM funcion f " +
                 "JOIN sala s ON f.ID_SALA = s.ID_SALA " +
                 "WHERE s.ID_SUCURSAL = (SELECT ID_SUCURSAL FROM sala WHERE ID_SALA = ?) " +
                 "AND f.ID_PELICULA = ? " +
                 "AND f.FECHA_FUNCION = ? " +
    ...
    try (Connection conn = ConexionBD.getConnection();
        ...
        ps.setInt(1, idSala);
        ps.setInt(2, ((Item)combo_pelicula.getSelectedItem()).getId());
        ...
        rs.next();
        if (rs.getInt(1) > 0) {
            JOptionPane.showMessageDialog(null, "Ya existe una función idéntica registrada");
            return true;
        }
    } catch (SQLException e) {
    ...
    // Segunda validación: sala ocupada en ese horario
    String sql2 = "SELECT COUNT(*) FROM funcion " +
                 "WHERE ID_SALA = ? AND FECHA_FUNCION = ? " +
                 "AND ((FUNCION_INICIO <= ? AND FUNCION_FIN > ?) OR " +
                 "(FUNCION_INICIO < ? AND FUNCION_FIN >= ?))";
              
    try (Connection conn = ConexionBD.getConnection();
         PreparedStatement ps = conn.prepareStatement(sql2)) {
        ps.setInt(1, idSala);
        ps.setDate(2, java.sql.Date.valueOf(fecha));
        ps.setTime(3, java.sql.Time.valueOf(horaFin + ":00"));
        ps.setTime(4, java.sql.Time.valueOf(horaInicio + ":00"));
        ps.setTime(5, java.sql.Time.valueOf(horaFin + ":00"));
        ps.setTime(6, java.sql.Time.valueOf(horaInicio + ":00"));
    ...
    private void initComponents() {
        ...
        jPanel1.add(nuevo, new org.netbeans.lib.awtextra.AbsoluteConstraints(970, 710, 150, 47));
    ...
    try (Connection conn = ConexionBD.getConnection();
         PreparedStatement ps = conn.prepareStatement("SELECT ESTADO_SALA FROM sala WHERE ID_SALA = ?")) {
        ps.setInt(1, idSala);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
            int estado = rs.getInt("ESTADO_SALA");
            if (estado != 1) {
                JOptionPane.showMessageDialog(this, "No se puede modificar la función a una sala deshabilitada");
                return;
            }
        }
    } catch (SQLException e) {
        JOptionPane.showMessageDialog(this, "Error al verificar estado de la sala: " + e.getMessage());
        return;
    }

    // El resto de tu código de modificar...
   ...
   if (JOptionPane.showConfirmDialog(this, "¿Está seguro de eliminar esta función?", 
    ...
    try (Connection conn = ConexionBD.getConnection();
        ...
        if (rs.next()) {
            int estado = rs.getInt("ESTADO_SALA");
            if (estado != 1) {
                JOptionPane.showMessageDialog(this, "No se puede asignar una función a una sala deshabilitada");
                return;
            }
        }
    } catch (SQLException e) {
    ...
    if (row != -1) {
        ...
        // Seleccionar la película correspondiente
        String nombrePelicula = tabla.getValueAt(row, 1).toString();
        for (int i = 0; i < combo_pelicula.getItemCount(); i++) {
            Item item = (Item) combo_pelicula.getItemAt(i);
            if (item.toString().equals(nombrePelicula)) {
                combo_pelicula.setSelectedIndex(i);
                break;
            }
        }
        
        // Seleccionar la sala correspondiente
        String numeroSala = tabla.getValueAt(row, 5).toString();
        for (int i = 0; i < combo_sala.getItemCount(); i++) {
            Item item = (Item) combo_sala.getItemAt(i);
            if (item.toString().equals("Sala " + numeroSala)) {
    ...
    // Resetear los combobox manteniendo la sucursal seleccionada
    int indexSucursal = combo_sucursal.getSelectedIndex(); // Guardamos el índice de la sucursal
    
    // Resetear sala y película
    if (combo_sala.getItemCount() > 0) {
        combo_sala.setSelectedIndex(0);
    }
    if (combo_pelicula.getItemCount() > 0) {
        combo_pelicula.setSelectedIndex(0);
    }
    
    // Deseleccionar la fila de la tabla si hay alguna seleccionada
    tabla.clearSelection();
